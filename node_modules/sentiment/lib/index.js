...
function isNumber(value) {
    ...
    // and then check if it's a valid number using isNaN
...
Sentiment.prototype.analyze = function (phrase, opts, callback) {
    ...
    while (i--) {
        ...
        if(isNumberX === true){ tokenScore = numberValue}
        // eslint-disable-next-line max-len
        tokenScore = languageProcessor.applyScoringStrategy(languageCode, tokens, i, tokenScore);
 
        if (tokenScore > 0 & isNumberX == false) positive.push(obj);
        if (tokenScore < 0 & isNumberX == false) negative.push(obj);
        score += tokenScore;
